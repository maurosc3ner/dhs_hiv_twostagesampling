...
colmat <- function(nquantiles = 3, 
  ...
  if (plotLeg) {
    p <- ggplot(col.matrix.plot, aes(X, Y, fill = HEXCode)) +
      ...
      theme(aspect.ratio = 1,
            axis.title = element_text(size = fontSize, colour = "black",hjust = 0.5,face = "italic", 
                                      vjust = 1),
  ...
  if (saveLeg) {
    ggsave(filename = "bivLegend.png", plot = p, device = "png",
...
bivariate.map <- function(rasterx, rastery, colormatrix = col.matrix,
                          nquantiles = 4, export.colour.matrix = TRUE,
